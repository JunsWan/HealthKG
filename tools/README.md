...
'''{
    ...
    history args:
    {
        "id": str,                              #知识图谱表示动作的id字段
        "timestamp" : "2025-12-08T21:48:31",    #时间 isoformat(timespec="seconds")
        "body_part" : str,                      #对应的部位
        "target_muscles" : List[str]            #目标肌肉群
    }

    # 注意其中需要让llm分析时确定body_part的部位限定在 ['Arm', 'Back', 'Calf', 'Chest', 'ForeArm', 'Hips', 'Neck', 'Should', 'Thigh', 'Waist']
    # equipemt的数值限定在 ["Barbell", "Dumbbell", "Cable", "Smith", "Lever", "Suspended", "Sled", "Band Resistive"]
'''
...
包含字段
        {
            ...
            "name": r.get("name"),                          #名称
            "summary": r.get("instructions", ""),           #动作详细介绍（包括准备动作和训练动作要领，可以直接反馈给用户，或者让大模型转化成更方便阅读的形式）
            "fields": {
                "body_part": target_body_part,              #目标部位
                "utility": r.get("utility"),                
                "force": r.get("force"),
                "target_muscles": r.get["target_muscles"]   #目标肌肉
            },
